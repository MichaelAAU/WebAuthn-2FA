 ...
 * while missing: authenticator options, extra credentials and timeout
 * @param  {String} username       - username
 * @param  {String} displayName    - user's personal display name
 * @param  {String} id             - user's base64url encoded id
 * @return {{attestation: string, challenge: String, user: {displayName: String, name: String, id: String}, rp: {name: string}, pubKeyCredParams: {type: string, alg: number}[]}} - server encoded make credentials request
 */
...
/**
 * Generates makeCredentials request with UV
 * while missing: extra credentials and timeout
 * @param  {String} username       - username
 * @param  {String} displayName    - user's personal display name
 * @param  {String} id             - user's base64url encoded id
 * @return {{attestation: string, challenge: String, user: {displayName: String, name: String, id: String}, rp: {name: string}, pubKeyCredParams: {type: string, alg: number}[]}} - server encoded make credentials request
 */
let generateServerMakeCredRequestUV = (username, displayName, id) => {
    return {
        challenge: randomBase64URLBuffer(32),
...
/**
 * Generates getAssertion request with UV
 * @param  {Array} authenticators              - list of registered authenticators
 * @return {PublicKeyCredentialRequestOptions} - server encoded get assertion request
 */
let generateServerGetAssertionUV = (authenticators) => {
    let allowCredentials = [];
    for(let authr of authenticators) {
        allowCredentials.push({
            type: 'public-key',
...
module.exports = {
    ...
    generateServerMakeCredRequestUV,
    generateServerGetAssertion,
    generateServerGetAssertionUV,
